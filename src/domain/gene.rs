...
impl Evolution for Vec<Gene> {
    ...
    fn recalculate_fitness(mut self) -> Self {
        self.iter_mut().for_each(|gene| {
            ...
            gene.fitness = match new_distance < 100f64 {
                false => {
                    (DISTANCE_REWARD / new_distance)
                        + sf_first_prize
                        + sf_last_prize
                        + uniqueness_prize
                }
    ...
    fn crossing_over(self) -> Self {
        ...
        let aux_pop = (1..self.len()).fold(Vec::new(), |new_pop, _| {
            let p1 = self
                .choose_multiple(&mut rng, CROSSING_OVER_CANDIDATES)
                .map(|g| g.clone())
            ...
            [
                ...
                vec![Gene {
                    cities: [
                        p1.cities[0..middle_gene].to_vec(),
                        p2.cities[middle_gene..gene_size].to_vec(),
                    ]
                    .concat(),
                    fitness: 0f64,
                    distance: 0f64,
...
impl std::string::ToString for Gene {
